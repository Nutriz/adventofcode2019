...
data class Point2D(val x: Int, val y: Int) {
    ...
    fun distanceTo(point2D: Point2D): Int {
        return abs(x - point2D.x) + abs(y - point2D.y)
    }
...
fun main() {
    val (wire1Input: String, wire2Input: String) = inputsDay3.split("\n")

    val wire1 = parseWire(wire1Input)
    val wire2 = parseWire(wire2Input)

    val intersections = wire1.intersect(wire2)
    val part1 = intersections.map { it.distanceTo(Point2D.ORIGIN) }.min()
    println("part1: $part1")

    val part2 = intersections.map { cross ->
